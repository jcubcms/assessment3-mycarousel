...
function my_shortcode($atts, $content = null) {
    ...
    return my_frontend($atts);
}
add_shortcode('image-carousel', 'my_shortcode');
...
function my_frontend($atts){
    ...
    while ( $loop->have_posts() ) {
        ...
        if ( '' != get_the_post_thumbnail(get_the_ID(), $atts['image_size']) ) {
            ...
            $url = get_post_meta(get_the_ID(), 'my_image_url', true);
            $url_openblank = get_post_meta(get_the_ID(), 'my_image_url_openblank', true);
            $link_text = get_post_meta(get_the_ID(), 'my_image_link_text', true);
            $video_url = get_post_meta(get_the_ID(), 'my_video_url', true);
            $video_aspect = get_post_meta(get_the_ID(), 'my_video_aspect', true);
            $images[] = array(
                'post_id' => $post_id,
                'title' => $title,
                'content' => $content,
    ...
    if(count($images) > 0){
        ob_start();
        ?>
        <div id="my_<?php echo $id; ?>" class="carousel slide<?php if($atts['use_carousel_fade'] == '1'){ echo ' carousel-fade'; } ?>" <?php if($atts['use_javascript_animation'] == '0'){ echo ' data-ride="carousel"'; } ?> data-interval="<?php echo $atts['interval']; ?>">
            ...
            // First content - the carousel indicators
                if( count( $images ) > 1 ){ ?>
                    ...
                    <?php foreach ($images as $key => $image) { ?>
                        <li data-target="#my_<?php echo $id; ?>" data-slide-to="<?php echo $key; ?>" <?php echo $key == 0 ? 'class="active"' : ''; ?>></li>
                    <?php } ?>
                    </ol>
                <?php
                }
            ...
            if( count( $images ) > 1 ){
                if( $atts['showcontrols'] === 'true' && $atts['twbs' ] == '3') { ?>
                    <a class="left carousel-control" href="#my_<?php echo $id; ?>" data-slide="prev"><span class="glyphicon glyphicon-chevron-left"></span></a>
                    <a class="right carousel-control" href="#my_<?php echo $id; ?>" data-slide="next"><span class="glyphicon glyphicon-chevron-right"></span></a>
                <?php } elseif ( $atts['showcontrols'] === 'true' && $atts['twbs'] == '4' ) { ?>
                    <a class="carousel-control-prev" role="button" href="#my_<?php echo $id; ?>" data-slide="prev"><span class="carousel-control-prev-icon icon-prev"></span></a>
                    <a class="carousel-control-next" role="button" href="#my_<?php echo $id; ?>" data-slide="next"><span class="carousel-control-next-icon icon-next"></span></a>
                <?php } elseif ( $atts['showcontrols'] === 'true' ) { ?>
                    <a class="left carousel-control" href="#my_<?php echo $id; ?>" data-slide="prev">‹</a>
                    <a class="right carousel-control" href="#my_<?php echo $id; ?>" data-slide="next">›</a>
                <?php } elseif ( $atts['showcontrols'] === 'custom' && $atts['twbs'] == '3' && $atts['customprev'] != '' &&  $atts['customnext'] != '' ) { ?>
                    <a class="left carousel-control" role="button" href="#my_<?php echo $id; ?>" data-slide="prev"><span class="<?php echo $atts['customprev'] ?> icon-prev"></span></a>
                    <a class="right carousel-control" role="button" href="#my_<?php echo $id; ?>" data-slide="next"><span class="<?php echo $atts['customnext'] ?> icon-next"></span></a>
                <? } elseif ( $atts['showcontrols'] === 'custom' && $atts['twbs'] == '4' && $atts['customprev'] != '' &&  $atts['customnext'] != '' ) { ?>
                    <a class="carousel-control-prev" role="button" href="#my_<?php echo $id; ?>" data-slide="prev"><span class="<?php echo $atts['customprev'] ?> icon-prev"></span></a>
                    <a class="carousel-control-next" role="button" href="#my_<?php echo $id; ?>" data-slide="next"><span class="<?php echo $atts['customnext'] ?> icon-next"></span></a>
                <?php }
            } ?>
        ...
        <script type="text/javascript">
            jQuery(document).ready(function() {
                jQuery('#my_<?php echo $id; ?>').carousel({
        ...
        <?php }
                ...
                if (count($video_providers) > 0) { ?>
                    <script>
                        var $myCarousel = jQuery('#my_<?php echo $id; ?>');
        ...
        $output = '<!-- CPT Bootstrap Carousel - no images found for #my_'.$id.' -->';
    }
